...
type AppData = { children: Child[]; schedules: ScheduleItem[]; checks: PostVaccinationCheck[] }
...
function loadData(): AppData { try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) return JSON.parse(raw) } catch {} return { children: [], schedules: [], checks: [] } }
...
function App() {
  ...
  const [mode, setMode] = useState<'home' | 'child' | 'detail' | 'postCheck'>('home')
  ...
  return <main className="app">
    <header className="hero-card"><p className="eyebrow">기능 테스트 MVP</p><h1>우리 아이 접종·검진 체크</h1><p>자녀별 일정을 만들고, 접종 완료 24시간 뒤 접종열/컨디션 체크까지 기록합니다.</p><div className="stat-row"><span>자녀 {data.children.length}</span><span>예정 {stats.pending}</span><span>완료 {stats.completed}</span><span>상태기록 {stats.checks}</span></div></header>
    ...
    <nav className="tabs"><button className={mode === 'home' ? 'active' : ''} onClick={() => setMode('home')}>통합 홈</button><button className={mode === 'child' ? 'active' : ''} onClick={() => setMode('child')} disabled={!selectedChild}>아이별 일정</button></nav>
